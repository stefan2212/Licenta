...
Ultimul termen va fi referit ca fracția continuă asociată fracției $\frac{a}{b}$ și va finotat prin $[ q_1,q_2, \dots ,q_{k+1}]$. Expresiile $[q_1,q_2, \dots ,q_i]$ , $1 \leqslant i \leqslant k+1$, vor fi numite convergențele fracției continue $[q_1,q_2, \dots ,q_{k+1}]$.\\
...
$$Pr_{P^{'}}[ cmmdc(2P^{'},P^{'}) =2 ]$$
$$ = Pr_{P^{'}} [2 \nmid P^{'} \wedge 3 \nmid P^{'}  \dots    \wedge B \nmid P^{'} ] =(1- \frac{1}{2})(1 - \frac{1}{3}) \dots (1- \frac{1}{B}) = \prod_{p_i \leqslant B}^{} (1- \frac{1}{P^i}) \approx \frac{1}{e^{\lambda} \ln(B)}  $$
...
\end{enumerate} 
	 ...
	 Căutăm să găsim acel $a$, astfel încât $ a = a_i \ mod \ m_i $ pentru $i$ de la $1$ până la $k$. Să dovedim că $ a = \sum_{i=0}^{k} a_i \cdot   M_i \cdot   y_i $ este o soluție a sistemului. \\
	 $M_i= \prod_{j=0,j \neq i}^{k} m_j $ și $M_j=0 \ mod \ m_i $ dacă $j \neq i$, de aceea: 
	 $$ a = a_i \cdot   M_i \cdot   y_i \ mod \ m_i $$
	 Dar am ales $y_i$ astfel încât $y_i = M_{i}^{-1} \ mod \ m_i$ ; în alte cuvinte $y_i \cdot   M_i = 1 \ mod \ m_i$. În final, obținem pentru i de la 1 până la k :
	 $$ a = a_i \  mod \ m_i$$
	 ...
	 \subsection{Algoritmul lui Garner}
	  ...
	  \section{RSA Rebalansat}
	   \subsection{Generarea cheilor, criptare și decriptare}
	    ...
	    $$d\textprime = \frac{d_P-a}{2} + k_1 \cdot   \frac{P-1}{2} $$
	    $$d\textprime = \frac{d_Q-a}{2} + k_2 \cdot   \frac{Q-1}{2} $$
		...
		\section{Multi-Power RSA}
		 \subsection{Hensel lifting}
		  ...
		  \begin{algorithm}[H]
		   ...
		   \subsubsection{Descrierea algoritmului}
		   În acest algoritm vom primi ca și input $M_1,M_2, \dots M_p$, $p$ exponenți $e_1, \dots ,e_p$ și modulul $N$. Reprezentarea binară a fiecăruia este cunoscută: $e_i=(e_{i,k-1}, \dots e_{i,0})_2$. Calculăm $C=M_{1}^{e_1}* \dots *M_{p}^{e_p}$. Acest calcul este mult mai rapid decât daca am face exponențieri consecutive.
		   ...
		   $$b:i=(i_{p-1} \dots i_0)_2 -> \prod_{j=0}^{p-1} M_{j}^{i_j} \ mod \ N$$
